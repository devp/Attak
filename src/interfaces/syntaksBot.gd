...
# extension and resigning beats hanging the game.
...
# Returns false when the engine cannot take this game. syntaks plays 6x6 at one
# fixed komi and nothing else.
...
func _chooseMove(state: GameState) -> Ply:
	if _engine == null:
		return await _giveUp("syntaks is not loaded")
	...
	if not _engine.start_search(state.getTPS(), nodes, millis):
		return await _giveUp("syntaks refused the position %s" % state.getTPS())
	...
	if _engine.is_searching():
		return await _giveUp("syntaks search timed out after %.0fs" % SEARCH_TIMEOUT_SECONDS)

	var ptn: String = _engine.take_result()
	if ptn.is_empty():
		return await _giveUp("syntaks returned no move")

	var ply := Ply.fromPTN(ptn)
	if ply == null:
		return await _giveUp("could not parse syntaks' move '%s'" % ptn)

	return ply


# Any engine failure is a bug. Returning null makes BotInterface concede, which
# ends the game visibly rather than stalling the board.
func _giveUp(reason: String) -> Ply:
	push_error(reason)
	Notif.message("The bot hit an error and resigned.")
	await get_tree().process_frame
	return null
