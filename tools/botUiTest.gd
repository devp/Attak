...
# game going with the bot on one side and a reply coming back. Where the syntaks
# GDExtension is absent, checks the tab is hidden instead.
...
func _ready() -> void:
	...
	if panel != null and button != null:
		if not SyntaksBot.available():
			# No engine, no bot: the tab must be hidden rather than offered and broken.
			if button.visible:
				_fail("syntaks is unavailable but the Vs Bot tab is still shown")
			else:
...
func _startGameAndCheckReply(panel: Node) -> void:
	...
	panel.diffEntry.select(0)   # Syntaks (fast)
	...
	var reply := _firstEmptyFlat(GameLogic.activeState())
	if reply == null:
		_fail("no empty square to reply on after the bot's opening")
		return
...
# Any flat on an empty square is legal on ply 1, which is all the reply needs.
func _firstEmptyFlat(state: GameState) -> Ply:
	for x in state.size:
		for y in state.size:
			if state.board[x][y].is_empty():
				return Place.new(Vector2i(x, y), Place.TYPE.FLAT)
	return null
